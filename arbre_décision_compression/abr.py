...
def dot(Noeud):
   ...
   '''
   for node in listN:
      f.write(node[0].get_id() + "   [ label=\" " + node[0].valeur[0:2] + " \"];\n")
      f.write(node[1].get_id() + "   [ label=\" " + str(node[1].valeur)[0:2] + " \"];\n")
      f.write(node[2].get_id() + "   [ label=\" " + str(node[2].valeur)[0:2] + " \"];\n")
...
'''
   for i in range(len(listN)-1):
       for j in range(i+1,len(listN)-1):
            if listN[i][0] == listN[j][0]:
                listN[i].pop(0)
                #del listN[i]

       f.write(listN[i][0].get_id() + "   [ label=\" " + listN[i][0].valeur + " \"];\n")
       f.write(listN[i][1].get_id() + "   [ label=\" " + str(listN[i][1].valeur) + " \"];\n")
       f.write(listN[i][2].get_id() + "   [ label=\" " + str(listN[i][2].valeur) + " \"];\n")
       f.write(listN[i][0].get_id() + " -> " + str(listN[i][1].get_id()) + "   [ style=dashed  ];\n")
       f.write(listN[i][0].get_id() + " -> " + str(listN[i][2].get_id()) + "   [ style=solid  ];\n")
   f.write("}")
